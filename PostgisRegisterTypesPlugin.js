...
const plugin = builder => {
    ...
    builder.hook("init", (_, build) => {
            ...
            if (!pgTweaksByTypeIdAndModifer[typeId][typeModifierKey]) {
                pgTweaksByTypeIdAndModifer[typeId][typeModifierKey] = (fragment, _resolveData) => {
                    const params = [
                        ...
                        sql.fragment `${sql.identifier(POSTGIS.namespaceName || "postgis", "postgis_type_name" // MUST be lowercase!
                        )}(
                ${sql.identifier(POSTGIS.namespaceName || "postgis", "geometrytype" // MUST be lowercase!
                        )}(${fragment}),
                ${sql.identifier(POSTGIS.namespaceName || "postgis", "st_coorddim" // MUST be lowercase!
                        )}(${fragment}::text)
              )`,
                        sql.literal("__srid"),
                        sql.fragment `${sql.identifier(POSTGIS.namespaceName || "postgis", "st_srid" // MUST be lowercase!
                        )}(${fragment})`,
                        sql.literal("__geojson"),
                        sql.fragment `${sql.identifier(POSTGIS.namespaceName || "postgis", "st_asgeojson" // MUST be lowercase!
                        )}(${fragment})::JSON`,
                    ];
        ...
        pg2GqlMapper[GEOGRAPHY_TYPE.id] = {
            map: identity,
            unmap: (o) => sql.fragment `st_geomfromgeojson(${sql.value(JSON.stringify(o))}::text)::${sql.identifier(POSTGIS.namespaceName || "postgis", "geography")}`,
        };
